...
[在线使用](https://yun532.github.io/LACT-observation-plan/) · [源表来源](docs/DATA.md) · [恒星目录](docs/STARS.md) · [计算口径](docs/ASTRONOMY.md)
...
亮星采用 V 波段，默认 V≤3，最高可显示到 V≤8。目录包含 47,114 条 Gaia DR3 / SIMBAD 恒星记录，进入单夜页后才载入；有自行数据的记录按其坐标历元传播。邻近图同样显示亮星，默认在原 5° 参考天区外扩 3°，外围范围可调；设置的视场半径另画蓝色虚线。LHAASO 的实测 r39 与上限用不同线型，不统一解释 TeVCat 定义不明的角尺度。恒星标记用于触发环境检查，未计入自动排班可观测条件，也不代表已建立触发噪声模型。
