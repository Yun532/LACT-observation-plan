# LACT 观测规划台

面向 LACT 的 TeV 源查询与观测规划网站。先比较每个源的全年观测窗口，再进入同一晚的轨迹、邻近源与任务排班；附带不使用太阳／月亮限制的 LHAASO 几何过境模式。

LHAASO 模式用于年度／月度源表统计；单夜排班与指向输出始终使用 LACT 观测条件。LHAASO 的过境可见性不会被当作望远镜指向任务。

[在线使用](https://yun532.github.io/LACT-observation-plan/) · [源表来源](docs/DATA.md) · [私有目录](docs/PRIVATE_CATALOG.md) · [恒星目录](docs/STARS.md) · [计算口径](docs/ASTRONOMY.md)

**Fermi 扩展源库**位于页面顶部和单夜工作台，按需载入正式 4FGL-DR4 v35 的 7,195 条公开记录。可按名称／别名、CLASS1 类型、显著性、FHL 高能关联及台站几何可达范围筛选；每页 25 条，能流只在同能段的 Fermi 库内排序。点击“加入源表”后，该源才出现在年度表、天空分布、当晚选源和排班中；“单夜观测”可直接加入并查看轨迹。已选记录以紫色显示，选择保存在本机。完整来源、字段与适用范围见 [FERMI.md](docs/FERMI.md)。

源表名称旁的 ⓘ、天图源卡和单夜“源表信息”现在打开同一份结构化信息卡：类别解释、别名／关联、坐标系、形态、带单位与能段的流强、谱指数、目录统计、探测器分量，以及折叠的原始字段。沿用 Neutrino-Sky 的信息组织思路，使用本项目各公开目录的可追溯字段；不复制其混合目录导出或自动匹配结果。非公开二期模型字段仍仅在浏览器本地呈现。

天空分布提供 Mollweide 等面积全天图、赤道／银道坐标切换、银道、月度时长着色、缩放平移和联动源列表。浅绿色填充站址的「可观测天区」：以 J2000 目录赤纬作静态近似，表示每日过境时满足天顶角限制的总范围，随台站纬度及天顶角上限变化。经度影响过境时刻，不改变这一全天范围；固定赤纬边界在赤道 Mollweide 中是水平线，切换银道坐标后通常呈曲线。默认叠加橙色的 LHAASO 50° 比较天区，以固定站址（纬度 29.3586111°、经度 100.1374972°）和天顶角上限 50° 得到赤纬 −20.64°～+79.36°；实线边界与当前设置的绿色虚线区分，改变当前站址或天顶角不会移动这一比较范围。蓝色填充当前预览源周围的球面视场，半径可调，支持放大和查看视场内源中心。三个区域顶部均有独立开关，色块统一放在下方图例，仅显示已开启的图层；均不代表已经满足指定夜晚的暗夜和日月条件。单夜计算包含岁差与章动，边界附近可能略有差异。悬停查看信息，点击源进入单夜；列表可定位目标。视图菜单保留「经典坐标图 · 1.0」。月度颜色固定在 0–240 h（更高值封顶），确切时长见源卡片；灰色表示尚未计算，不能当作零。亮星的上下两张单夜天图各有独立开关，关闭邻近图亮星时同时隐藏恒星明细。

改造前版本已保存为 **[v1.0.0](https://github.com/Yun532/LACT-observation-plan/tree/v1.0.0)**。需要恢复整站时按 [回退步骤](docs/ROLLBACK.md) 从 Actions 发布该标签；无需删除新代码。[天图设计与公开背景调研](docs/SKY_ATLAS.md) 说明参考项目及后续 Fermi、HAWC、Planck 等数据候选，本轮没有接入波段影像底图。

## 使用流程

1. 在「源与观测窗口」选择年份，搜索名称、别名或类型，按所选月／全年时长、个人优先级或 TeVCat 流强参考排序。源表热图展示每月的可观测小时数，也可切换天空分布。
2. 选择一个源和月份，查看逐日窗口，再进入指定夜晚。也可直接进入单夜工作台，点击输入框旁的「选择当晚源」，按所选夜晚的可观测时长从长到短浏览源及分段窗口，筛选目录、名称和状态，直接叠加或移除轨迹。列表与轨迹采用相同的 1 分钟采样及观测条件，时长未扣除已排任务。单夜工作台同时显示天顶角、月距、可观测时段、天顶角分布及邻近天区。
3. 将候选源拖入时间轴，移动任务或拖动边缘调整时长；也可通过表单精确输入。冲突检查包括可观测条件、任务重叠、最小时长与转场预留。同一源的相邻任务不收取换源转场时间。
4. 核对计划后下载任务 CSV、指向采样 CSV 或完整 JSON。草稿 JSON 可重新导入，继续编辑。参数设置可修改站址、时区、太阳与月亮限制、天顶角、视场和排班偏好。

网站在浏览器本地计算，无须登录或后端服务。个人标记及草稿保存在当前浏览器；换设备或清理浏览器前请导出 JSON。年度计算在 Web Worker 中运行，不会把源列表发送给远程计算服务。

完整 Fermi 库用于检索，只有已选条目参与观测计算，避免几千个源挤满默认天图。引用 Fermi 的草稿会按目录源标识恢复，不采用导入计划内自带的替代坐标；使用中的任务会阻止移除相应源。若 Fermi 文件暂时加载失败，基础目录仍可查询，原草稿暂停写入，重试成功后恢复。目录关联和几何可观测时长均不代表已经在 TeV 波段检出或可由 LACT 探测。

非公开的二期 LHAASO 源表通过网页内的本地导入使用：在当前浏览器中替换一期目录，并保留公开 TeVCat 源表。原文件和转换后的科学数据均不随网站上传至 GitHub 或 Pages；其他访问者仍看到公开目录。导入时可选择“导入后记住此设备”，将目录原始文本保存在当前浏览器的 IndexedDB；这个选项仅在选择文件导入时生效。私有目录的草稿和优先级按目录内容隔离；恢复公开源表会移除已记住的私有目录，“清除本机私有数据”还会清理其草稿与重点标记。详见 [私有目录说明](docs/PRIVATE_CATALOG.md)。

单夜图的时间滑块同时更新轨迹读数与台站天空图。天空图以天顶为中心、地平线为外圈，北上东左；可开关整夜轨迹、源展宽和亮星。日出日落单独标注，暗夜起止使用设置中的太阳高度阈值及裁剪时间。月距限制在天空图中标出，太阳不满足暗夜条件时提示整个天空不可用。

亮星采用 V 波段，默认 V≤3，最高可显示到 V≤8。目录包含 47,114 条 Gaia DR3 / SIMBAD 恒星记录，进入单夜页后才载入；有自行数据的记录按其坐标历元传播。邻近图同样显示亮星，默认在原 5° 参考天区外扩 3°，外围范围可调；设置的视场半径另画蓝色虚线。1LHAASO 的实测 r39 与上限用不同线型；本地二期高斯模型使用提供者确认的 σ 轮廓，95% 位置误差另行保留，不作为源展宽。不统一解释 TeVCat 定义不明的角尺度。恒星标记用于触发环境检查，未计入自动排班可观测条件，也不代表已建立触发噪声模型。

## 数据与时间口径

- 当前目录快照包含 **363 条 TeVCat 公开记录及 90 个 1LHAASO 源**。这是目录条目数，跨目录可能是同一物理源。默认显示已确立／新宣布条目，争议和候选源可通过筛选查看。
- 本地二期表的 `ra/dec` 按提供者确认的 J2000（FK5）度坐标读取；高斯空间模型的 `ext` 为二维高斯 σ（度）。半径 σ 的圆对称二维高斯轮廓约包容 39.35%，不等于 68% 区域，且不与 95% 定位误差混用。
- 数据为随版本发布的快照，获取日期、原始出处及完整性统计保存在 `public/data/sources.json` 的 `meta` 中；源名称、别名、坐标和科学量保留出处。缺失值不填零，未探测分量保留上限标志。
- TeVCat 的 Crab 单位流强存在不同能阈和观测历元；1LHAASO 的 WCDA／KM2A 归一化流强具有不同参考能量和单位。不能将所有原始数值混排为统一亮度。
- 默认站址：纬度 **29.3586111° N**、经度 **100.1374972° E**、海拔 **4410 m**、固定时区 **UTC+8**。默认天顶角上限 70°、太阳高度上限 −13°、月距下限 40°，来自原观测 Notebook。
- 单夜详细图为本地 **18:00 至次日 08:00**，采用 1 分钟步长。年度／月度累计采用 10 分钟中点积分，实际计算每个日历日；LACT 以当地中午至次日中午为一个观测日，LHAASO 以 00:00–24:00 为一个日历日。
- 固定 UTC 偏移不会自动应用夏令时。改变站址后，单夜 14 小时视窗未必覆盖当地所有暗夜，全天累计应查看月度／年度结果。

更多分量选择、流强、扩展尺度及数据重建说明见 [DATA.md](docs/DATA.md)。算法、坐标系、采样与参考对照见 [ASTRONOMY.md](docs/ASTRONOMY.md)。

## 本地运行

安装 Node.js 24 和 npm，在仓库根目录运行：

```sh
npm ci
npm run dev
```

访问终端显示的本地地址。生产构建及预览：

```sh
npm test
npm run build
npm run preview
```

构建输出为 `dist/`。Vite 使用相对资源路径，支持部署到 GitHub Pages 的仓库子路径。开发依赖已锁定在 `package-lock.json`；运行时不依赖外部脚本 CDN。

## 更新源表

本节命令只用于**公开目录**。非公开目录应使用网页中的本地导入，不能写入 `public/`、`data/raw/`、构建产物或提交记录。改为 JSON、压缩或改名不会让非公开科学数据变成可公开数据。

源表结构为 `{ "meta": {...}, "sources": [...] }`。每条记录必须有稳定的字符串 `id`、`name`、`catalog`、度单位的数值 `ra/dec`；其他科学字段及原始分量保留在记录中。完整字段说明见 [DATA.md](docs/DATA.md)。

导入器只使用 Python 3 标准库。离线重建与在线刷新命令分别为：

```sh
python scripts/import_catalog.py --output public/data/sources.json
python scripts/import_catalog.py --refresh --output public/data/sources.json
```

离线重建会校验 `data/raw/` 中缓存快照的 SHA-256；刷新后需一并提交新快照、目录 JSON 和质量报告，并审阅变化。日常网站构建直接使用已提交的 JSON，不联网抓取目录。

## 部署

在仓库 **Settings → Pages → Build and deployment** 中选择 **GitHub Actions**。推送至 `main` 后，[部署工作流](.github/workflows/deploy.yml) 依次运行 `npm ci`、测试、构建，再将 `dist/` 发布至 GitHub Pages。Pull request 只执行测试和构建。

工作流使用 GitHub 官方 [自定义 Pages 工作流](https://docs.github.com/en/pages/getting-started-with-github-pages/using-custom-workflows-with-github-pages)，通过 `github-pages` 环境部署，不需要自建访问令牌。

## 适用范围

这里的「可观测时间」是满足几何约束的时长，未扣除天气、设备停机和实际探测效率。5 min 转场与 3° 视场半径是可调规划默认值，尚不是经核准的 LACT 设备模型。

指向导出包含 UTC／本地时间、ICRS 源中心坐标及无大气折射的几何高度／方位角；方位角自北向东增加，`END_EXCLUSIVE` 标记任务结束边界。未实现 wobble 指向、设备限位、转速／加速度、线缆缠绕、地形遮挡及控制系统协议，**导出文件不能直接作为望远镜硬件控制指令**。

临近目录位置用于辅助选择和对比，并不单独给出源混淆或污染结论。判断污染还需要扩展形态、PSF、背景区域和具体分析方法。

## 检查与维护

`npm test` 使用 Node 原生测试运行器，无额外测试框架。测试覆盖采样与 Astropy 参考值的角度对照、闰年与跨年、极昼、LHAASO 全天统计、任务重叠与转场、时区换日、计划导入和 CSV 公式防护。参考值是原 Notebook 站址下四个源、三个夜晚的固定样本，不能替代真实设备验收。

天文计算依赖 [Astronomy Engine](https://github.com/cosinekitty/astronomy)；目录学术引用与数据出处见 [DATA.md](docs/DATA.md)。
