...
天空分布提供 Mollweide 等面积全天图、赤道／银道坐标切换、银道、月度时长着色、缩放平移和联动源列表。浅绿色填充站址的几何可达天区；蓝色填充当前预览源周围的球面视场，半径可调，支持放大和查看视场内源中心。二者独立开关，均不代表已经满足指定夜晚的暗夜和日月条件。悬停查看信息，点击源进入单夜；列表可定位目标。视图菜单保留「经典坐标图 · 1.0」。月度颜色固定在 0–240 h（更高值封顶），确切时长见源卡片；灰色表示尚未计算，不能当作零。亮星的上下两张单夜天图各有独立开关，关闭邻近图亮星时同时隐藏恒星明细。
